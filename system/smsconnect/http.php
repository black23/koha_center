...
class Http
{
    ...
    function clear()
    {
        ...
        $this->userAgent    = 'CSMSConnect PHP (v2.0)';
        $this->interface    = null;
     ...
     * Set extra headers
     *
     * @param array witch special headers
     * @return void
     */
     ...
     * Whether to save persistent cookies in subsequent calls
     *
     * @param varchar ip adresss interface
     * @return void
     */
    ...
    function execute($target = '', $referrer = '', $method = '', $data = array())
    {
		...
		$this->params = $data;//Když to není pole a prostě chci narvat do celého POSTu třeba xml string
        }
	    ...
	    if (is_array($this->params))
                $queryString = $this->_makeQueryString( $this->params );
	    else
        ...
        if ($this->useCurl)
        {
            ...
            if( is_array( $this->extraHeaders ) && !empty( $this->extraHeaders ) )
            ...
            for ( $i=0; $i < $count; $i++ ) {
                if (strtolower(substr($contentArray[$i], 0, 4 )) == 'http') {
                    $start = $i;
                }
            }
            ...
            // Store the contents
            $this->result = $contentArray[$start + 1];

            // Parse the headers
            $this->_parseHeaders($contentArray[$start + 0]);
    ...
    /**
     * Created Query string (internal)
     *
     *
     * @param array pole k zakodovani do query string
     * @return string
     * @access private
     */
    function _makeQueryString( $data, $path = array() )
    {
        $tempString = array();
        ...
        if( is_array( $data ) && count( $data ) > 0 )
        {
            foreach( $data as $key => $value)
            {
                if( is_array($value) )
                {
                    $tempString[] = $this->_makeQueryString( $value, array_merge( $path, array( $key ) ) );
                }
                else
                {
                    $real_path = array_merge( $path, array( $key ) );
                    $real_key = '';
                    foreach( $real_path as $index => $name)
                        $real_key .= ( $index ? '[' : '' ) . $name . ( $index ? ']' : '' );
                    $tempString[] = urlencode( $real_key ) . "=" . urlencode( $value );
                }
            }
        }
        return implode('&', $tempString);
    }
    ...
    function _parseHeaders($responseHeader)
    {
        ...
        {
            // Oooops !
            if(!preg_match("%^http\/[0-9]+\.[0-9]+\s+([0-9]+)\s+(.*)$%i", $headers[0], $matches))
            {
