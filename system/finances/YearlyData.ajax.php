...
# along with Koha; if not, see <http://www.gnu.org/licenses>.
    ...
    function createQuery($db, $departments, $month, $year, &$results) {
        $tempFrom = $year."-".$month."-01";
        $tempTo = date("Y-m-t", strtotime($tempFrom));
        $query = "SELECT round(Sum(ABS(`s`.`value`)),0) as `value`, `a`.`accounttype`, '$year-$month' as `age` "
				."FROM `statistics` `s` "
				."JOIN `accountlines` `a` "
    ...
    for($i = 1; $i <= $no_months; $i++) {
        ...
        createQuery($db, $myArray, $tempMonth, $tempYear, $results);
    }
