...
# along with Koha; if not, see <http://www.gnu.org/licenses>.
	...
	try {
        ...
        $query = "SELECT `a`.`accounttype`, `a`.`timestamp`, `b`.`borrowernumber`, `b`.`firstname`, `b`.`surname`, round(Sum(ABS(`s`.`value`)),0) as `value` "
				...
				."WHERE `s`.`branch` = :department AND DATE(`s`.`datetime`) BETWEEN :from AND :to AND `s`.`type` = :type AND `a`.`accounttype` IN ('".implode('\',\'', $cat_other)."') "	
...
$html = "
<h3>$text->other ($dayFrom - $dayTo) - $departmentText</h3>
<table>
        <thead>
            <tr>
                <th><b>$text->name</b></th>
