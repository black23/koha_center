...
# along with Koha; if not, see <http://www.gnu.org/licenses>.
	...
	try {
        ...
        $query = "SELECT `a`.`timestamp`, `b`.`borrowernumber`, `b`.`firstname`, `b`.`surname` "
				...
				."WHERE `s`.`branch` IN ('".implode('\',\'', $myArray)."') AND DATE(`s`.`datetime`) = :date AND `s`.`type` = :type AND `a`.`accounttype` IN ('".implode('\',\'', $cat_registrations)."') "	
...
$html = "
<h3>$text->registered ($day) - $departmentText</h3>
<table>
        <thead>
            <tr>
                <th><b>$text->name</b></th>
