...
# along with Koha; if not, see <http://www.gnu.org/licenses>.
	...
	try {
        ...
        $query = "SELECT `s`.`datetime`, `b`.`borrowernumber`, `b`.`firstname`, `b`.`surname`, count(`b`.`borrowernumber`) AS `count` "
				...
				."WHERE `s`.`branch` IN ('".implode('\',\'', $myArray)."') AND DATE(`s`.`datetime`) BETWEEN '$from' AND '$to' AND `s`.`type` = 'issue' "
...
$html = "
<h3>$text->borrowed ($dayFrom - $dayTo) - $departmentText</h3>
<table>
        <thead>
            <tr>
                <th><b>$text->name</b></th>
                <th><b>$text->surname</b></th>
                <th><b>$text->count</b></th>
                <th><b>$text->time</b></th>
            </tr>
        </thead>
        <tbody>
