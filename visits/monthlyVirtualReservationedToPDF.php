...
# along with Koha; if not, see <http://www.gnu.org/licenses>.
	...
	try {
        ...
        $query = "SELECT `a`.`timestamp`, `b`.`borrowernumber`, `b`.`firstname`, `b`.`surname` "
				...
				."WHERE `s`.`branch` IS NULL AND DATE(`s`.`datetime`) BETWEEN '$from' AND '$to' AND `s`.`type` = :type AND `a`.`accounttype` IN ('".implode('\',\'', $cat_reservations)."') "	
...
$html = "
<h3>$text->reserved_online ($dayFrom - $dayTo)</h3>
<table>
        <thead>
            <tr>
                <th><b>$text->name</b></th>
