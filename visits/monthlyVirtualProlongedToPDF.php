...
# along with Koha; if not, see <http://www.gnu.org/licenses>.
	...
	try {
        ...
        $query = "SELECT `s`.`datetime`, `b`.`borrowernumber`, `b`.`firstname`, `b`.`surname`, count(`b`.`borrowernumber`) AS `count` "
				...
				."WHERE `s`.`branch` IS NULL AND DATE(`s`.`datetime`) BETWEEN '$from' AND '$to' AND `s`.`type` = 'renew' "
...
$html = "
<h3>$text->prolonged_online ($dayFrom - $dayTo)</h3>
<table>
        <thead>
            <tr>
                <th><b>$text->name</b></th>
...
// This method has several options, check the source code documentation for more information.
$pdf->Output('prolongedonline.pdf', 'I');
