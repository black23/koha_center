...
# along with Koha; if not, see <http://www.gnu.org/licenses>.
	...
	try {
        ...
        $query = "SELECT `a`.`accounttype`, `a`.`timestamp`, `b`.`borrowernumber`, `b`.`firstname`, `b`.`surname` "
				."FROM `statistics` `s` "
                ."JOIN `accountlines` `a` "
                ."ON `a`.`accountlines_id` = `s`.`other` "
                ."LEFT OUTER JOIN `borrowers` `b` "
                ."ON `b`.`borrowernumber` = `a`.`borrowernumber` "
				."WHERE `s`.`branch` IN ('".implode('\',\'', $myArray)."') AND DATE(`s`.`datetime`) BETWEEN '$from' AND '$to' AND `s`.`type` = :type AND `a`.`accounttype` IN ('".implode('\',\'', $cat_allPayments)."') "	
    ...
    foreach($results as $key => $val){
        $results[$key]["accounttype"] = getAccountType($results[$key]["accounttype"], $text);
    }
...
$html = "
<h3>$text->paid ($dayFrom - $dayTo) - $departmentText</h3>
<table>
        <thead>
            <tr>
                <th><b>$text->name</b></th>
